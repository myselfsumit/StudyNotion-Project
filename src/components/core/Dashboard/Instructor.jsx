  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Safeguard before setting state: Check if data is valid
    ...
    fetchData();
  }, [token]); // Dependency array to refetch data on token change

  // Calculate total amount and total students if instructorData exists
  const totalAmount = instructorData?.length
    ? instructorData
        .filter((course) => course.totalAmountGenerated)
        .reduce((acc, curr) => acc + curr.totalAmountGenerated, 0)
    : 0;

  const totalStudents = instructorData?.length
    ? instructorData
        .filter((course) => course.totalStudentsEnrolled)
